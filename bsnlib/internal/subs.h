...
/** matrix subs */
template <typename S, typename A, typename B> class Operator<S, Operation<S, A, B, OperationsType::M_SUB_M>>;

/** matrix subs traits */
template <typename S, typename A, typename B> struct traits<Operator<S, Operation<S, A, B, OperationsType::M_SUB_M>>> {
    enum{
        rows = traits<A>::rows,
        cols = traits<B>::cols
    };
...
/** matrix subs operation implementation */
template <typename S, typename A, typename B> struct OperationImplementation<S, A, B, OperationsType::M_SUB_M> {
    static S eval(const A& a, const B& b, const unsigned int i, const unsigned int j)     {
        return a(i, j) - b(i, j);
    }
};

/** matrix subs operator implementation */
template <typename S, typename A, typename B> class Operator<S, Operation<S, A, B, OperationsType::M_SUB_M>> {
public:

    typedef Operation<S, A, B, OperationsType::M_SUB_M> BaseOperation;
    ...
    S operator ()(const unsigned int i, const unsigned int j) const     {
        ...
        return OperationImplementation<S, A, B, OperationsType::M_SUB_M>::eval(a, b, i, j);
    }
